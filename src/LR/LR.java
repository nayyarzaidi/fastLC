...
public abstract class LR {
	...
	class ObjectiveFunction implements DifferentiableFunction {
		...
		public void hessian(int ii, double[] hessian) {
			...
			computeHessian(inst, probs, x_C, hessian);
		}
	...
	public abstract void computeHessian(Instance inst, double[] probs, int x_C, double[] gradients);
	...
	public void train() {
		...
		if (m_O.equalsIgnoreCase("QN")) {
			...
			MinimizerSGD alg = new MinimizerSGD();
			alg.setMaxIterations(maxIterations);
			Result result;	

			if (is_Verbose) {
				int method = 5;
				System.out.print("fx_SGD_" + name + "_" + method + " = [");
				result = alg.run(function_to_optimize, parameters, method);
				System.out.println("];");
				System.out.println("NoIter = " + result.iterationsInfo.iterations);
				System.out.println();
			} else {
			...
			System.out.println("Only QN, CG, GD, Tron, LBFGS, SGD are implemented");
			System.exit(-1);
		} 
